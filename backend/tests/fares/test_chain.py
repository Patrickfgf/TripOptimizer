...
def test_first_provider_wins() -> None:
    ...
    live = _Fixed(CELL, Fare("LIS", "BCN", dt.date(2026, 7, 1), 99.0, "EUR", "travelpayouts"))
    chain = FallbackFareProvider([cached, live])
...
def test_falls_through_to_second_on_miss() -> None:
    ...
    chain = FallbackFareProvider([cached, live])
    fare = chain.get_fare(*CELL)
    assert fare.price == 99.0 and fare.source == "travelpayouts"
