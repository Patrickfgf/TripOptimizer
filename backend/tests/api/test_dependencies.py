...
from tripoptimizer.core.fares.on_demand import SafeLiveProvider
...
def test_get_provider_without_token_is_snapshot_only(monkeypatch) -> None:
    # Real-or-nothing: no token -> serve only the committed snapshot. No live layer,
    # and crucially NO synthetic fallback -- an unpriced cell stays unpriced.
    monkeypatch.delenv("TRAVELPAYOUTS_TOKEN", raising=False)
    dependencies.get_provider.cache_clear()
    try:
        provider = get_provider()
        assert [type(p) for p in provider._providers] == [CachedProvider]
    finally:
        dependencies.get_provider.cache_clear()
...
def test_get_provider_inserts_live_layer_with_token(monkeypatch) -> None:
    monkeypatch.setenv("TRAVELPAYOUTS_TOKEN", "dummy")
    dependencies.get_provider.cache_clear()
    dependencies.get_fare_cache.cache_clear()
    try:
        provider = get_provider()
        # The live layer is SafeLiveProvider(CachingMonthProvider(...)) after the snapshot.
        assert any(isinstance(p, SafeLiveProvider) for p in provider._providers)
    finally:
        dependencies.get_provider.cache_clear()
        dependencies.get_fare_cache.cache_clear()
